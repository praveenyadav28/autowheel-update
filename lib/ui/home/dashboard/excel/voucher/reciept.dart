...
Future<void> createExcelReciept(
  dataList, {
  ...
  sheet?.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 0)).value =
      TextCellValue('RV Number');
  sheet?.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: 0)).value =
      TextCellValue('RV Date');
  ...
  for (var data in dataList) {
    int voucherId = data['voucher_Mode_Id'];
    String voucherName =
        modeList!.firstWhere((element) => element['id'] == voucherId)['name'];
    sheet!
        .cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rowIndex))
        .value = TextCellValue("${data['rv_No']}");
    sheet
        .cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: rowIndex))
        .value = TextCellValue(data['receipt_Date']);
    sheet
    ...
    sheet
        ...
        .value = TextCellValue(data['cash_Amount']);
    sheet
