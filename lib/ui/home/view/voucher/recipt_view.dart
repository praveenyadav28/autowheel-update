...
import 'package:autowheel_workshop/ui/home/dashboard/excel/voucher/reciept.dart';
import 'package:autowheel_workshop/utils/components/library.dart';
import 'package:intl/intl.dart';

class RecieptView extends StatefulWidget {
  RecieptView({required this.reportType, super.key});
...
class _RecieptViewState extends State<RecieptView> {
  ...
  void initState() {
    ...
    fatchVoucher().then((value) => setState(() {
          widget.reportType
              ? null
              : getReciept(dateFrom: datepickar1.text, dateTo: datepickar2.text)
                  .then((value) => setState(() {
                        filteredList = recieptList;
                      }));
        }));
    super.initState();
  }
  ...
  void filterList(String searchText) {
    setState(() {
      filteredList = recieptList.where((value) {
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  createExcelReciept(filteredList, modeList: voucherList);
                },
                icon: const Icon(Icons.document_scanner))
          ],
          title: Text(widget.reportType ? "Receipt Report" : "Receipt View",
                              ...
                              children: [
                                Container(
                                    ...
                                    child: ListTile(
                                      dense: true,
                                      title: Text(
                                        "${filteredList[index]["receipt_Date"]}"
                                            .substring(
                                                0,
                                                filteredList[index]
                                                            ["receipt_Date"]
                                      ...
                                      trailing: Text(
                                        "Reciept Voucher No : ${filteredList[index]['rv_No']}",
                          ...
                          children: [
                            ...
                            ...List.generate(filteredList.length, (index) {
                              ...
                              return TableRow(
                                  ...
                                  children: [
                                    tableRow("${filteredList[index]["rv_No"]}"),
                                    tableRow(
                                        filteredList[index]['ledger_Name']),
                                    tableRow(
                                        "${filteredList[index]["receipt_Date"]}"
  ...
  //Reciept
  Future getReciept({required String dateFrom, required String dateTo}) async {
    ...
    var response = await ApiService.fetchData(
        "Transactions/GetReceiptVoucherALLocationwiseAW?datefrom=$formattedDateFrom&dateto=$formattedDateTo&locationid=${Preference.getString(PrefKeys.locationId)}&StaffId=0");
    setState(() {
      recieptList = response;
      filteredList = recieptList;
    });
  }
  ...
  Future deleteReciept(int? voucherNo) async {
    var response = await ApiService.postData(
        "Transactions/DeleteReceiptVoucherAW?prefix=online&refno=$voucherNo&locationid=${Preference.getString(PrefKeys.locationId)}",
        {});
...
//Reciept Datails Get
  Future fatchRecieptDetails(int? rvNumber) async {
    recieptDetails = await ApiService.fetchData(
        'Transactions/GetReceiptVoucherAW?prefix=online&refno=$rvNumber&locationid=${Preference.getString(PrefKeys.locationId)}');
  }
