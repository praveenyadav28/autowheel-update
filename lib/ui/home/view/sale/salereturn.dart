...
import 'package:autowheel_workshop/ui/home/dashboard/excel/sale/sale_return.dart';
import 'package:autowheel_workshop/utils/components/library.dart';
import 'package:intl/intl.dart';

class SaleReturnView extends StatefulWidget {
  SaleReturnView({required this.reportType, super.key});
...
class _SaleReturnViewState extends State<SaleReturnView> {
  ...
  void filterList(String searchText) {
    setState(() {
      filteredList = saleReturnList.where((value) {
        // Ledger
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  createExcelSaleReturn(filteredList, ledgerList: ledgerList);
                },
                icon: const Icon(Icons.document_scanner))
          ],
          title: Text(
              widget.reportType ? "Sale Return Report" : "Sale Return View",
                              ...
                              children: [
                                Container(
                                    ...
                                    child: ListTile(
                                      ...
                                      trailing: Text(
                                        "Sale Return No : ${filteredList[index]['pRtn']}",
                          ...
                          children: [
                            ...
                            ...List.generate(filteredList.length, (index) {
                              ...
                              return TableRow(
                                  ...
                                  children: [
                                    tableRow("${filteredList[index]["pRtn"]}"),
                                    tableRow(ledgerName),
                                    tableRow(
                                        "${filteredList[index]["pRtn_Date"]}"
                                            .substring(
  ...
  Future getSaleReturn(
    ...
    var response = await ApiService.fetchData(
        "Transactions/GetSaleReturnALLocationwiseAW?datefrom=$formattedDateFrom&dateto=$formattedDateTo&locationid=${Preference.getString(PrefKeys.locationId)}&StaffId=0");
    saleReturnList = response;
    filteredList = saleReturnList;
  }

  //Delete Sale Return
  Future deleteSaleReturn(int? invoiceNo) async {
    var response = await ApiService.postData(
        "Transactions/DeleteSaleReturnAW?prefix=online&refno=$invoiceNo&locationid=${Preference.getString(PrefKeys.locationId)}",
        {});
...
//fatch Purchase Invoice Data
  Future getSaleReturnData(pRtnNo) async {
    pRtnDetails = await ApiService.fetchData(
        "Transactions/GetSaleReturnAW?prefix=online&refno=$pRtnNo&locationid=${Preference.getString(PrefKeys.locationId)}");
  }
...
//Get ledger List
  Future fatchledger() async {
    var response = await ApiService.fetchData(
        "MasterAW/GetLedgerByGroupId?LedgerGroupId=10,23,24,25,29,9,7,8,11");
