...
import 'package:autowheel_workshop/ui/home/dashboard/excel/sale/estimate.dart';
import 'package:autowheel_workshop/utils/components/library.dart';
import 'package:intl/intl.dart';

class EstimateView extends StatefulWidget {
  EstimateView({required this.reportType, super.key});
...
class _EstimateViewState extends State<EstimateView> {
  ...
  void initState() {
    ...
    widget.reportType
        ? null
        : getAllEstimate(dateFrom: datepickar1.text, dateTo: datepickar2.text)
            .then((value) => setState(() {
                  filteredList = estimateList;
                }));
    super.initState();
  }
  ...
  void filterList(String searchText) {
    setState(() {
      filteredList = estimateList.where((value) {
    ...
    return Scaffold(
      appBar: AppBar(
          actions: [
            IconButton(
                onPressed: () async {
                  createExcelEstimate(filteredList);
                },
          ...
          centerTitle: true,
          title: Text(widget.reportType ? "Estimate Report" : "Estimate View",
                              ...
                              children: [
                                Container(
                                    ...
                                    child: ListTile(
                                      ...
                                      trailing: Text(
                                        "Doc No : ${filteredList[index]['doc_No']}",
                                ...
                                ListTile(
                                    ...
                                    title: Row(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          "   (${filteredList[index]['doc_Date'].substring(0, filteredList[index]['doc_Date'].length - 12)})",
                                          style: rubikTextStyle(
                                              14,
                                              FontWeight.w400,
                                              AppColor.colGrey),
                                    ...
                                    trailing: PopupMenuButton(
                                      onSelected: (value) async {
                                        if (value == 'Generate') {
                                          ...
                                          pushTo(Estimate(
                                              items: const [],
                                              docNumber: filteredList[index]
                                                  ['doc_No']));
                                        } else if (value == 'Delete') {
                                          deleteEstimate(
                                                  filteredList[index]['doc_No'])
                                              .then((value) => getAllEstimate(
                                                    dateFrom: datepickar1.text,
                                                    dateTo: datepickar2.text,
                                                  ).then((value) =>
                                                      setState(() {})));
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        IconButton(
                                            onPressed: () {
                                              deleteEstimate(filteredList[index]
                                                      ['doc_No'])
                                                  .then((value) =>
                                                      getAllEstimate(
                                                        dateFrom:
  ...
  Future getAllEstimate(
    ...
    var response = await ApiService.fetchData(
        "Transactions/GetEstimateALLocationwiseAW?datefrom=$formattedDateFrom&dateto=$formattedDateTo&locationid=${Preference.getString(PrefKeys.locationId)}&StaffId=0");
    estimateList = response;
    filteredList = estimateList;
  }

  //Delete Estimate
  Future deleteEstimate(int? invoiceNo) async {
    var response = await ApiService.postData(
        "Transactions/DeleteEstimateAW?prefix=online&refno=$invoiceNo&locationid=${Preference.getString(PrefKeys.locationId)}",
        {});
...
//Fatch Estimate Details
  Future getEstimateDetails(int? refNo) async {
    invoiceDetails = await ApiService.fetchData(
        "Transactions/GetEstimateAW?refno=$refNo&prefix=online&locationid=${Preference.getString(PrefKeys.locationId)}");
  }
}
